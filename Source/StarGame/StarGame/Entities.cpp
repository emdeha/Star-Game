...
void Satellite::Render(glutil::MatrixStack &modelMatrix, ProgramData &data)
{
...
{
	glm::vec4 lightPosCameraSpace = modelMatrix.Top() * glm::vec4(position, 1.0f);

	// TODO: Containing lighting data
	// Should be put in a 'Light' class for more flexibility
	glUseProgram(data.theProgram);
	glUniform4f(data.lightIntensityUnif, 1.0f, 1.0f, 1.0f, 1.0f);
	glUniform4f(data.ambientIntensityUnif, 0.2f, 0.2f,  0.2f, 0.2f);
	glUniform3fv(data.cameraSpaceLightPosUnif, 1, glm::value_ptr(lightPosCameraSpace));
	glUniform1f(data.lightAttenuationUnif, 1.2f);
	glUniform1f(data.shininessFactorUnif, 0.5f);
	glUniform4fv(data.baseDiffuseColorUnif, 1, glm::value_ptr(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f)));
	...
	{
		satellites[i]->Render(modelMatrix, data);
	}
}
void Sun::Update()
{
...
void Sun::AddSatellite(const std::string &fileName, float height, float offset, float speed)
{
	Satellite *sat = new Satellite(Framework::Timer(Framework::Timer::TT_LOOP, speed), height, offset);
