...
void Scene::RenderScene(glutil::MatrixStack &modelMatrix, float interpolation)
{
	...
	SpriteParticleData spriteParticleData = shaderManager.GetSpriteParticleProgData();
	...
	for(int i = 0; i < sizeSuns; i++)
	{
		suns[i]->Render(modelMatrix, materialBlockIndex, sceneGamma, 
						litData, unLitData, simpleData, textureData, litTextureData, spriteParticleData,
						interpolation);
	}
}
